...
public class Hil2PromelaVisitor extends aVisitor {
	...
	public AcceptReturnType visitCompositeStateNode(CompositeStateNode tNode) {
		...
		/* 
		 * perl ASTVisitorForPromela::visitCStateNode
         * TODO verify the following:
		#directly output the empty proctype to @GlobaloutputCState
		#haven't done yet   [what????!!!! [KDL]]	
		 */
	}
	...
	public AcceptReturnType visitEventNode(EventNode tNode) {
		...
		/*if (tNode.getParent() == null) return tmpART;
		if (tNode.getParent().getParent() == null) return tmpART;*/
        ...
        if (grandpaType.equals("TransitionNode")) {
            if (tNode.getEventType().equals("normal")) {
                ...
                if (tmpStateTimeInvariant.length() > 0) {
                    ...
                    tmpStr += strln("        :: atomic{Timer_V." + tmpStateTimeInvariant + " && " + classRefID
                              + "_q?" + tNode.getName() + " ->");
                } else {
                ...
                if (tNode.getVariable().length() > 0) {
                    ...
                    /*    my $temp1=$classname."_q";    my $temp2=$classname."_";
                          my $temp3=$eventname."_p1";   my $temp4=$classname."_V.";                     */
                    // push(@outputState,"                   $temp2$temp3?$temp4$eventvar");
                    // push(@outputState,"                   -> ");
                    tmpStr += strln("                   " + classRefID + "_" + tNode.getName()
                              + "_p1?" + classRefID + "_V." + tNode.getVariable());
                    tmpStr += strln("                   -> ");
                    /* 
                     * perl
                     * 
	...
	public AcceptReturnType visitHistoryNode(HistoryNode tNode) {
        ...
        for (int i = 0; i < tCBN.children.size(); i++) {
          ...
          if (tNode.getID().equals(entity.getID())) {
            ...
            if ((entity.getType().equals("CompositeStateNode")) || (entity.getType().equals("ConcurrentCompositeNode"))) {
	...
	public AcceptReturnType visitInitNode(InitNode tNode) {
        ...
        if (tNode.hasTransitionBodyNode()) {
            ...
            /* (done) array join order is as follows: outputInit += "/ Initial actions.../"
             *   then UniversalClass->jointwoarrays effectively appends the output
             *   from the TransitionBodyNode *after* the live above. 
             *   
             */ 
       ...
       /* 
        String tNodeParentType = aParent.getType();
        while (aParent.getType() != tNodeParentType){
	...
	public AcceptReturnType visitInstanceVariableNode(InstanceVariableNode tNode) {
        ...
        if ((tNode.getInitValue().length() > 0)) { //TODO comparison may not be totally correct, type may need changing.
        	...
        	String tmpStr = strln ("        " + className + "_V." + tNode.getVar()
        					+ " = " + tNode.getInitValue() + ";");
        ...
        if (tNode.getSignalName().length() > 0) {
          if (tNode.getClassName().length() > 0) {
            ...
            if (tNode.getIntVarName().length() > 0) {
            	 ...
            	 * temp2 = $msgintvarname."; ".$temp5.$msgsignalname."};"
            	 *
            	 * 	my $temp1="atomic"."{";            	 * 	my $temp2=$msgclassname."_";
            	 * 	my $temp3=$msgsignalname."_p1!";   	 *  my $ClassParent=$classref->{ID};
            	 * 	my $temp4=$ClassParent."_V.";      	 * 	my $temp5=$msgclassname."_q!";
            	 * 
            	 */
...
//            		push(@outputmessage,"        $temp1$temp2$temp3$temp4$msgintvarname; $temp5$msgsignalname};");
            ...
            // T O D O(done) @outputmessage = visitmessageNodePak->outputOnePartmessage( $msgsignalname, @outputmessage );
	...
	public AcceptReturnType visitSignalNode(SignalNode tNode) {
        ...
        if (tNode.getSignalType().length() > 0){
            ...
            tmpART.addStr("Signal", "chan " + className + "_"+ tNode.getName() + "_p1=[5] of {" + tNode.getSignalType() + "};");
            /*
	...
	public AcceptReturnType visitStateNode(StateNode tNode) {
		...
		/* 
		 * perl  AST::visitStateNode
		#Actually only one child here, i.e., statebodyNode
		 */
	}
	...
	public AcceptReturnType cbnhCStateHeadOutput (aNode tNode, boolean isFromClassCall) {
        ...
        /* #SK: Fix for process die bug (applied by KL)
		if (isFromClassCall) { // minor cosmetic difference.
			tmpStr = "        " + tmpStr;
		}
        */
		...
		tmpART.addStr("CState", "        atomic{" + cstatename + "_C?1;" + padding1 + "wait??" + cstatename + "_pid,m;}");
		...
		for (int i = 0; i < tNode.children.size(); i++) {
			...
			for (int j = 0; j < bodyChild.children.size(); j++) {
				...
				if  ((childNode.getType().equals("StateNode"))               || (childNode.getType().equals("CompositeStateNode"))
				  || (childNode.getType().equals("ConcurrentCompositeNode")) || (childNode.getType().equals("TransitionNode"))) {
			...
			for (int i = 0; i < tNode.bodyNode.children.size(); i++) {
				...
				if (childNode.getType().equals("CompositeStateNode")) {
					...
					if (csNode.bodyNode != null) {
						/*
						 * TODO !!! important! (source of CCSB bugs?)
						 *    the CState version of the code calls UniversalClass->jointwostrings
						 *    which is NOT defined in universal Class!
						 *    Love Perl?
						*/
						...
						if (isFromClassCall) {
							tmpART.addStr("CCStateTemp1", "        " + temp20 + " = run " 
									+ csNode.getID() + "(none); " + temp30 + ";");							
						} else {
						...
						if (isFromClassCall) {
							tmpART.addStr("CCStateTemp2", "        atomic{" + temp40 + ";wait??" 
									+ temp20 + "," + temp30 + ";}");							
						} else {
...
//	public AcceptReturnType cbnhRunCProctype (ClassBodyNode tNode, AcceptReturnType anART) {
	public void cbnhRunCProctype (aNode tNode, aNode childNode, AcceptReturnType tmpART, boolean isFromClassCall) {
		...
		if (childNode.children.size() > 0) {
			...
			/*
			String temp100 = tmpART.getStr("CState");
			// TODO comparison algorithm may not be correct.
			// I think there's one like it, further down.
			String comparison = "        if";
			
			if (!temp100.substring(temp100.length() - comparison.length()).equals(comparison)) {
	...
	public void cbnhFormGlobalOutputInstVar (ClassBodyNode tNode, AcceptReturnType INPredicateTarget, AcceptReturnType tmpART) {
        ...
        for (int i = 0; i < tNode.children.size(); i++) {
          ...
          if (childNode.getType().equals("SignalNode")) {
            ...
            if (countSignalNumber == 1) {
              /* T O D O implement ->                  @GlobaloutputSignal =
          ...
          if (childNode.getType().equals("StateNode")) {
            tmpART.merge(childNode.accept(this)); 
            // T O D O implement ->   @outputWholeState = visitclassbodyNodePak->StateBlock( $ent, \@outputWholeState, \@outputState );
//            tmpART.merge(cbnhStateBlock(childNode));
          ...
          if (childNode.getType().equals("CompositeStateNode")) {
          	// T O D O implement -> visitclassbodyNodePak->RunCProctype( $ent, \@mtypelist, \@outputCStateID, \@outputWholeState );
        	  cbnhRunCProctype(tNode, childNode, tmpART, true);
          }
          if (childNode.getType().equals("ConcurrentCompositeNode")) {
            /*
             * T O D O implement ->            visitclassbodyNodePak->OutputCCState( $ent, \@mtypelist, \@outputCCStateIDint, \@outputCCStateIDmtype,
                                                  \@outputWholeState );
...
#To form the proctype output of this class to @outputClass
	...
	public void csbhStateBlock (aNode entNode, boolean ifHistory, AcceptReturnType tmpART) {
		...
		tmpART.addStr("WholeState", "/* State " + stateID + " */");
		tmpART.addStr("WholeState", stateID + ":    atomic{skip; printf(\"in state " 
			+ tParent.getID() + "." + stateID + "\\n\");" );
		
		if (ifHistory) {
			aNode cStateRef = searchUpForDest (entNode, "CompositeStateNode");
			tmpART.addStr("WholeState", "        H_" + cStateRef.getID() + " = st_" 
					+ stateID + ";  /* Save state for history */");
		}
	...
	public void csbhPushDownToStateNode (CompositeStateBodyNode tNode, StateNode SNode) {
		...
		*/
		...
		/*// No need to assign it to the children! the loop in the other code is just
		 * to get the StateNode's new/old body node! 
		for (int i = 0; i < SNode.bodyNode.children.size(); i++) {
			aNode childNode = (aNode) SNode.bodyNode.children.get(i);
		}
		*/
			...
			for (int i = 0; i < tNode.bodyNode.children.size(); i++) {
				...
				if (childNode.getType().equals("TransitionNode")) {
					if (((TransitionNode)childNode).getDestinationType().equals("Outgoing")) {
                        ...
                        String cStateID = ((TransitionNode)childNode).getFormerCStateParent().getParent().getID() + "_pid";
    ...
    public void csbhOutputTBridgeAndPID (AcceptReturnType anART) {
    	...
    	if (vec.size() > 0) {
        	...
        	for (int i = 0; i < vec.size(); i++) {
        		...
        		tempStr += strln ("        :: atomic{m == " + temp + " -> wait!_pid," + temp 
        							+ "; " + temp2 + "; goto exit; skip;};");
            ...
            for (int i = 0; i < tNode.children.size(); i++) {
                ...
                if (childNode.getType().equals("StateNode") ||
                    ...
                    childNode.getType().equals("ConcurrentCompositeNode")) {
                		...
                		String commonPart = "        :: H_" + tNode.getParent().getID() + " == "
                							  + "st_" + childNode.getID();
                		if (childNode.getType().equals("StateNode")) {
...
/*
 *  This is taken care of by CompositeStateNode.transitionNodeChildren
 *  ( $countTransNode, @OutgoingTransitionlist ) = visitcstatebodyNodePak->getOutgoingTransitions($thiscstatebodynode);
	...
	-> getOutgoingTransitions basically returns a list of all of the transitions in a
	composite state body node, because *all* of its transitions are outgoing.
	This is already handled by the cStateNode's transitionNodeChildren Vector.
 ...
 */
    ...
    visitcstatebodyNodePak->pushOutgoingTransitionlist( $thiscstatebodynode, $countTransNode, @OutgoingTransitionlist );
        ...
        for (int i = 0; i < tNode.children.size(); i++) {
          ...
          if (childNode.getType().equals("CompositeStateNode")) {
          	    /*  T O D O implement->          visitcstatebodyNodePak->RunCProctype( $ent, 
                 *          \@mtypelist, \@outputCStateID, \@outputWholeState,  
                 *          \@WholeOutgoingTransitionlist );
                */
          ...
          if (childNode.getType().equals("ConcurrentCompositeNode")) {
              /*
               * T O D O implement ->  visitcstatebodyNodePak->OutputCCState( $ent,
               *     \@mtypelist, \@outputCCStateIDint, 
          ...
          if (childNode.getType().equals("StateNode")) {
            ...
            /*
             * T O D O implement ->  @outputWholeState =
...
//		tmpART.merge(tNode.acceptChildrenOfType(this, "CompositeStateNode"));
		...
		/* **important**
		 *   According to perl source StateNode might be an illegal
		 *   type for children, while ActionNode is definitely
		 *   an illegal type for children.  However this code is
		 *   commented out.
		 * 
	...
	protected String mbnhMakeOut(int level, String txt) {
		...
		/*
		 *  Lesson Learned: forget the Lexer, forget even the parser,
		 *    forget Regular expression matching,
...
# then Put 0,text.
	...
	protected String mbnhChanGlobalOutput () {
		...
		tmpStr += mbnhPut (0, "chan wait=[10] of {int,mtype};");
	...
	protected String mtypeListOutput () {
        ...
        /*
         * Don't need to do this because it is implied
         *  that wholeleng % divisor = 0, since wholeleng = leng * divisor. 
        if (temp1.length() > 0) { // don't forget to add remaining!
          tmpout1.addElement(temp1);  
        }*/
	...
	protected String mbnhTokenGlobalOutput () {
		...
		tmpStr += mbnhPut (0, "chan t=[1] of {mtype};");
		tmpStr += mbnhPut (0, "mtype={free};");
		tmpStr += mbnhPut (0, "");
		tmpStr += mbnhPut (0, "");
		tmpStr += mbnhPut (0, "");
		tmpStr += mbnhPut (0, "");
...
# For Karli: I think you need to figure out what this dispatcher is actually good for. My guess if for some type of internal 


		 */
		/* commented out, please fix comment in line 3 if reinstated.
		tmpStr += mbnhPut (0, "");
		tmpStr += mbnhPut (0, "");
		tmpStr += mbnhPut (0, "/* This is the universal event dispatcher routine * /");
		tmpStr += mbnhPut (0, "proctype event(mtype msg)");
		tmpStr += mbnhPut (0, "{");
		tmpStr += mbnhPut (0, "   mtype type;");
		tmpStr += mbnhPut (0, "   int process_id;");
		tmpStr += mbnhPut (0, "");
		tmpStr += mbnhPut (0, "   atomic {");
		tmpStr += mbnhPut (0, "   do");
		tmpStr += mbnhPut (0, "   :: evq??[eval(msg),process_id] ->");
		tmpStr += mbnhPut (0, "      evq??eval(msg),process_id; ");
		tmpStr += mbnhPut (0, "      evt!msg,process_id;");
		tmpStr += mbnhPut (0, "      do");
		tmpStr += mbnhPut (0, "      :: if");
		tmpStr += mbnhPut (0, "         :: evq??[type,eval(process_id)] -> evq??type,eval(process_id)");
		tmpStr += mbnhPut (0, "         :: else break;");
		tmpStr += mbnhPut (0, "         fi");
		tmpStr += mbnhPut (0, "      od");
		tmpStr += mbnhPut (0, "   :: else -> break");
		tmpStr += mbnhPut (0, "   od}");
		tmpStr += mbnhPut (0, "exit:skip");
		tmpStr += mbnhPut (0, "}"); */
	...
	protected String mbnhTimerProcessOutput (AcceptReturnType tmpinfo) {
		...
		for (int i = 0; i < entries.length; i++) {
			tmpStr += mbnhPut (0, "                             if");
			tmpStr += mbnhPut (0, "                             :: Timer_V." + entries[i] + ">=0");
    ...
    @INPredicateTarget = ASTINPredVisitorForPromela->GetResult;
        ...
        //  @GlobalTimeVarSignal = visitclassbodyNodePak->FormTimingInstVar( $ent, \@GlobaloutputInstVarBody, \@INPredicateTarget, \@GlobaloutputInstVar, \@GlobalTimerInstVarOutput );
        ...
        /*
        tmpStr += tmpART.getStr("TimerInstVar");
        tmpART.removeStrKey("TimerInstVar");*/
    ...
    public AcceptReturnType sbnhOutput_Dest (StateBodyNode tNode, String dest, /*String optlDestID,*/ String destType, int countGuard, boolean checkMTypeList) {
    ...
    public AcceptReturnType sbnhOutput_Dest(StateBodyNode tNode, String transNodeDesc, String dest, /*String optlDestID,*/ String destType, int countGuard, boolean checkMTypeList) {
        AcceptReturnType tmpART = new AcceptReturnType();
    	...
    	if (countGuard == 0) {
    		tmpART.addStr("State", "           wait!_pid,st_" + dest + "; " + temp3.getID() + "_C!1; " + transNodeDesc + "goto exit; skip;}");
    	} else {
    		tmpART.addStr("State", "              wait!_pid,st_" + dest + "; " + temp3.getID() + "_C!1; " + transNodeDesc + "goto exit; skip;}");
    	}
    ...
    public AcceptReturnType sbnhTransitionEndOutput (aNode destNode) {
        ...
        if (stateTimeInvariant.length() > 0) {
        	/*
        	tmpART.addStr("timearray", stateTimeInvariant);
        	timeArray = tmpART.getStrSplitOnToken("timearray", "<=");
        	...
        	*/
        	...
        	if (!noOutput) {
        		...
        		tmpART.addStr("State", "           :: atomic{Timer_V." + timeArray[0] + filler1
        							    + timeArray[1] + " -> " + className + "_V.timerwait = 1;");
...
/* 
    ...
    public void sbnhOutputTransitions (StateBodyNode tNode, AcceptReturnType anART, AcceptReturnType transitionEventList, boolean ifTarget) {
    	AcceptReturnType TELentitity, transitionList;
...
//        	 safe to assume items are of type TransitionNode. (!?) (but is it transition node or transition body?
        	...
        	if (evtNodeSrc.equals("EMPTYTRANS")) {
        		...
        		for (int j = 0; j < tlvec.size(); j++) {
            		...
            		// anART.merge(sbnhOutput_Dest (tNode, transNode.getDestination(), transNode.getDestinationType(), 0, false));
        	...
        	if (evtNode != null) { // will only happen if neither of the two options above are true.
        		...
        		if (evtNode.getEventType().equals("normal")) {
        			...
        			if (localRet != null) {
        				...
        				anART.addStr("State", "        :: atomic{" + transitionMarkerStr + "evt??" + evtNode.getName() + ",eval(_pid) -> ");        				
        			}
        		} else {
        			anART.moveStrKey("Trans", "State");
        		}
        	...
        	if (!emtypTrans) {
        		...
        		for (int j = 0; j < tlvec.size(); j++) {
                    ...
                    // code modified 11/10/05
        			// anART.merge(sbnhOutput_Dest(tNode, transNode.getDestination(), transNode.getDestinationType(), countGuardVal, true));
...
/*        	if (tbNode.eventNodeChild == null) { //T ODO double check implementation, implement properly.
        		// sub EMPTYTRANSoutputState
        		String stTimeInvariant = stateTimeInvariant;
        		stateTimeInvariant = "";
        		
        		for (int j = 0; j < tbNode.transitionList.size(); j++) {
            		if (stTimeInvariant.length() == 0) {
            			anART.addStr("State", "        :: atomic{1 -> ");            			
            		} else {
            			anART.addStr("State", "        :: atomic{Timer_V." + stTimeInvariant + " -> ");            			
            		}
            		// T ODO finish.
            		anART.merge(sbnhOutput_Dest (tNode, "", "", false));
        		}
        	} else {
	...
	public AcceptReturnType visitStateBodyNode(StateBodyNode tNode) {
		...
		boolean inPredicateTarget = sbnhIfINPredicateTarget(tNode, (AcceptReturnType) globalOutputs.getSingle("INPredicateTarget"));
        ...
        if (tNodeCount == 0) { // has no transitions, only actions
            //T O D O (done) call  @outputState = visitstatebodyNodePak->NoTransitionOutput( $thisstatebodynode, @outputState );
            tmpStr += strln("        if");
            tmpStr += strln("        :: skip -> false");
            tmpStr += strln("        fi;");
            tmpART.addStrln("State", tmpStr);
            ...
            if (inPredicateTarget) {
                ...
                tmpStr += strln("        "+ tCN.getID() + "outputPredicateStatement_V.st_" + tNode.getParent().getID() + " = 1;");              
            }
            ...
            for (int i = 0; i < vec.size(); i++) {
                //TransitionBodyNode childNode = (TransitionBodyNode) vec.get(i); // safe to assume items are of type TransitionNode.
                ...
                if (TELentitity.getStr("event").substring(0, 2).equals("e_")) {
                      ...
                      if (evtNode.getType().equals("normal")) {
                    	  ...
                    	  // T O D O implemented -> my $returnvalue=UniversalClass->FindLocalDestNode($thisstatebodynode,"SignalNode",$event->{eventname},"name");
            ...
            // @outputState = visitstatebodyNodePak->outputTransitions( $thisvisitor, \@outputState, \@transeventlist, \@mtypelist, $iftarget,$thisstatebodynode );
