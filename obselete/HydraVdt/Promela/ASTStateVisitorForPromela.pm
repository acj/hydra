...
#added 31/7/2002 4:13PM
sub ifinarray
{
    my ($class,$outputstring,@outputCState)=@_;
    
    my @temparray;
    
    my $leng=scalar(@outputCState);
    if ($leng eq 4)
    {
        return 0;
    }
    else #($leng gt 4)
    {
        my $ent;
        my $counter=0;
        foreach $ent (@outputCState)
        {
            $counter=$counter+1;
            if ($counter gt 4)
            {#get the array entries after "        if"
                push(@temparray,$ent);
            }
        }
    }   

#search if $outputstring has already in @temparray (actually @outputCState) or not
    my $found=0;
...
sub visitTransNode 
{
    ...
    if ($SorCSorCCS ne '')  #$dest is in the subtree of $CSNode (excluding $CSNode), I don't need to output here
    {
        ...
        #modified 31/7/2002 11:48AM
            ...
            $found=ASTStateVisitorForPromela->ifinarray($outputstring,@outputCState);
            if ($found eq 0)
            {
                push(@outputCState,$outputstring);
            }
        #modified end
